...
export class AuthorizeUserDialogComponent extends BaseDialog<AuthorizeUserDialogComponent> implements OnInit {
  ...
  async ngOnInit() {
    this.spinner.show(SpinnerType.BallElasticDot)
    this.assignedRoles = await this.userService.getRolesToUser(this.data, () => this.spinner.hide(SpinnerType.BallElasticDot));
  ...
  assignRoles(rolesComponent: MatSelectionList) {
    ...
    this.userService.assignRoleToUser(this.data, roles,
      () => {
        this.spinner.hide(SpinnerType.BallElasticDot);
      }, error => {
